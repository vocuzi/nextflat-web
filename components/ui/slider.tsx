...
>(({ className, ...props }, ref) => (
    ...
    >
        ...
        {((props.value as number[]) || (props.defaultValue as number[]) || [0]).map((_, index) => (
            <SliderPrimitive.Thumb
                key={index}
                className="block h-5 w-5 rounded-full border-2 border-slate-900 bg-white ring-offset-white transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-900 focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 cursor-pointer shadow-sm hover:scale-110 active:scale-95"
            />
        ))}
    </SliderPrimitive.Root>
))
