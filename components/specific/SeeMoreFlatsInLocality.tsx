...
export default function SeeMoreFlats({
        ...
        const fetchFlats = async () => {
            try {
                const res = await fetch(
                    `https://v1apinffk.svc.nextflat.in/api/flats/search?state=0&city=${cityCode}&page_size=4&exclude=${excludeId}&localities=${localityName}`
                )
    ...
    return (
        <section className="py-10">
            ...
            <CityFlatsList
                flats={flats}
            // parentLabel={localityName}
            // parentSlug={locationSlug}
            // pagination={false} // Disable pagination for this component
            />
